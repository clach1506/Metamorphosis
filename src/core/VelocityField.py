...
class VelocityField:
    ...
    @classmethod
    def zeros(
        cls, T: int, height: int, width: int, kernel: GaussianKernel, device=None
    ) -> "VelocityField":
    ...
    def upsampled(self, size, kernel: GaussianKernel) -> "VelocityField":
        ...
        new_h, new_w = size
        w_x = ResolutionPyramid.upsample_stack(self.w_x.detach(), size) * (
            new_w / old_w
        )
        w_y = ResolutionPyramid.upsample_stack(self.w_y.detach(), size) * (
            new_h / old_h
        )
    ...
    def velocity_at(self, t: int):
        # v(t) = K^(1/2) w(t)
        return self.kernel.sqrt_smooth(self.w_x[t]), self.kernel.sqrt_smooth(
            self.w_y[t]
        )
