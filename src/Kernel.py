...
class GaussianKernel:
    ...
    @classmethod
    def at_scale_fraction(cls, frac: float, height: int, width: int, min_sigma: float = 2.0) -> "GaussianKernel":
        # Keeps sigma a fixed fraction of the image size across pyramid levels.
        return cls(max(min_sigma, frac * max(height, width)))

    def smooth(self, field: torch.Tensor) -> torch.Tensor:
        # v = K * w
        return self._separable_conv(field, self.sigma)
