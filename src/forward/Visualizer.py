...
class MetamorphosisVisualizer:
    ...
    def summary(self) -> str:
        ...
        disp = np.sqrt(
            (m.v_traj_x.sum(0) * self.dt) ** 2 + (m.v_traj_y.sum(0) * self.dt) ** 2
        )
        ...
        if final_diff.max() == 0:
            lines.append(
                "  (exactly 0 because a(T) is anchored to a(1) in the exact/collocation "
                "scheme -- not a measure of fit quality; see E_data/rms above instead)"
            )

        if m.s_traj is not None:
            dice = self._dice_score(m.s_traj[-1], m.s_target)
            dice_pure = self._dice_score(
                m.pure_deformation_segmentation_trajectory()[-1], m.s_target
            )
            lines += [
                "",
            ...
            if dice == 1.0:
                lines.append(
                    "  (first one trivially 1.0 -- S(T) is anchored to S(1) just like a(T)/a(1) "
    ...
    def export_all(self, output_dir: str):
        ...
        if self.m.history is not None:
            labels = (
                ("loss", "E_kinetic", "E_data", "E_seg")
                if self.m.s_traj is not None
                else ("loss", "E_kinetic", "E_data")
            )
            self.plot_loss_history(
                self.m.history, f"{output_dir}/fig_loss.png", labels=labels
            )
        if self.m.s_traj is not None:
